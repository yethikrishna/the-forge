...
func init() {
	rootCmd.AddCommand(cloneBehaviorCmd)

	cloneBehaviorCmd.AddCommand(cloneRecordCmd)
	cloneBehaviorCmd.AddCommand(cloneCommandCmd)
	cloneBehaviorCmd.AddCommand(cloneReadCmd)
	cloneBehaviorCmd.AddCommand(cloneWriteCmd)
	cloneBehaviorCmd.AddCommand(cloneEditCmd)
	cloneBehaviorCmd.AddCommand(cloneDecisionCmd)
	cloneBehaviorCmd.AddCommand(cloneSearchCmd)
	...
	cloneBehaviorCmd.PersistentFlags().StringVarP(&cloneOutput, "output", "o", "text", "output format: text, json")

	cloneRecordCmd.Flags().StringVar(&cloneDescription, "desc", "", "task description")

	cloneDecisionCmd.Flags().StringVar(&cloneReason, "reason", "", "reasoning for the decision")

	cloneCommandCmd.Flags().Duration("duration", 0, "command duration")
}
...
var cloneCommandCmd = &cobra.Command{
	Use:   "command [command]",
...
var cloneReadCmd = &cobra.Command{
	Use:   "read [file-path]",
...
var cloneWriteCmd = &cobra.Command{
	Use:   "write [file-path]",
...
var cloneEditCmd = &cobra.Command{
...
var cloneDecisionCmd = &cobra.Command{
...
var cloneSearchCmd = &cobra.Command{
	Use:   "search [query]",
...
func setActiveRecorder(r *clonebehavior.Recorder) {
	activeRecorder = r
}

func getActiveRecorder() *clonebehavior.Recorder {
	return activeRecorder
}

func clearActiveRecorder() {
	activeRecorder = nil
}

func getCloneStore() *clonebehavior.Store {
	return cloneStoreInst
}

func init() {
	cloneEditCmd.Flags().String("old", "", "old text being replaced")
	cloneEditCmd.Flags().String("new", "", "new text being inserted")
}
