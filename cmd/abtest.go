  ...
  forge abtest list`,
	...
	createCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(vf) < 2 { return fmt.Errorf("need >= 2 variants") }
			var vs []abtestpkg.Variant
			for _, v := range vf {
				p := splitVar(v)
				if len(p) != 2 { return fmt.Errorf("bad variant: %s", v) }
	...
	recordCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if v == "" { return fmt.Errorf("--variant required") }
	...
	analyzeCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil { return err }
			if len(exp.Results) == 0 { fmt.Println(pretty.InfoLine("No results")); return nil }
			a := abtestpkg.Analyze(exp)
			j, _ := cmd.Flags().GetBool("json")
			if j { data, _ := json.MarshalIndent(a, "", "  "); fmt.Println(string(data)); return nil }
	...
	listCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			exps, err := store.List()
			if err != nil { return err }
			if len(exps) == 0 { fmt.Println(pretty.InfoLine("No experiments")); return nil }
...
func splitVar(s string) []string {
	for i, c := range s { if c == ':' { return []string{s[:i], s[i+1:]} } }
	return nil
}
