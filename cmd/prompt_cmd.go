...
func init() {
	promptCmd.AddCommand(prRegisterCmd)
	promptCmd.AddCommand(prListCmd)
	promptCmd.AddCommand(prShowCmd)
	promptCmd.AddCommand(prRenderCmd)
	promptCmd.AddCommand(prSearchCmd)
	promptCmd.AddCommand(prForkCmd)
	promptCmd.AddCommand(prDefaultsCmd)
	promptCmd.AddCommand(prCategoriesCmd)

	promptCmd.PersistentFlags().StringVar(&prDir, "dir", ".forge/prompts", "Prompt registry directory")
	...
	prSearchCmd.Flags().String("query", "", "Search query")
}
...
var prForkCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fork, err := reg.Fork(args[0], args[1])
		if err != nil {
			return err
		}
